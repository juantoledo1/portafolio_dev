...
const Navigation: React.FC = () => {
  ...
  return (
    <>
      {/* GRADIENTE DE SEGURIDAD SUPERIOR (Para visibilidad) */}
      ...
      <nav className={`fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-[1000] transition-all duration-500 rounded-2xl border ${
        ...
        <div className="px-6 md:px-10 flex items-center justify-between">
          ...
          >
            <div className="w-8 h-8 bg-cyan-600 rounded-lg flex items-center justify-center font-bold text-white shadow-[0_0_15px_rgba(8,145,178,0.5)] group-hover:scale-110 transition-transform">
              J
            </div>
            <span className="text-white font-bold text-lg md:text-xl font-mono-custom tracking-tighter group-hover:text-cyan-400 transition-colors">
          ...
          <div className="hidden md:flex items-center gap-1 bg-slate-950/20 p-1 rounded-xl border border-white/5">
            {links.map((link) => (
              <button
                ...
                className="text-slate-300 hover:text-white hover:bg-white/5 px-4 py-2 rounded-lg text-sm font-medium transition-all font-mono-custom cursor-pointer"
              >
                {link.name}
              </button>
            ))}
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <button 
              ...
              className="px-5 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all hover:shadow-[0_0_15px_rgba(8,145,178,0.4)] cursor-pointer"
            >
              Contactar
            </button>
          </div>
      ...
      >
        <div className="p-8 flex-1">
          <div className="flex justify-between items-center mb-12">
            <span className="text-cyan-500 font-mono-custom font-bold">MENÚ</span>
